...
- **The README has no screenshot block, and the community listing reads the README.** No `images/screenshots/` and no capture suites exist here yet; G102 puts the gallery between the lead paragraph and `## Demo vault`. Wanted before the first release.
- **The demo-vault asset is `file-bundles-demo-vault.zip`, unversioned**, and it unzips into one `file-bundles-demo-vault-<version>` folder. The version moved out of the archive name and into the archive with ODU 98 (this repo is on 101); both READMEs name the new form. Do not reintroduce the versioned asset name — no release produces it.

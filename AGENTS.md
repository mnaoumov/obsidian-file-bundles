...
- **An entry that resolves to the vault root, or above it, is rejected too.** `normalizePath()` leaves a folder path as written — it keeps a trailing slash, answers `.` for an empty remainder and `..` for a climb past the top — so `/` reached the rest of the plugin as the vault ROOT, `/shared/brand/` named no folder the vault has, and `../..` named a path outside the vault altogether. `normalizeResolvedPath()` settles the canonical form once, for `main`, `files` and `folders` alike, and reports `EntryIsTheVaultRoot` / `EntryIsOutsideTheVault` like any other malformed entry. Rejecting it for one key only would have left the same hazard live one line over.
...
- **`alwaysUpdateLinks` reaches the device from the harness now.** Moving a note whose frontmatter links its own dependents otherwise raises Obsidian's "Update links" sheet, and the rename SITS THERE waiting for an answer: the main file lands in its new folder, the members never follow, and the next frame is photographed with the sheet across it. Before `obsidian-integration-testing` 17 the Android transport overwrote the `app.json` the harness had written, so every suite set the option itself; 17 fixed that, and the four in-suite workarounds are gone. `shoot` refuses to capture while any modal is open, so a regression cannot come back silently.
...
- **The declaring note is one of a bundle's OWN files, never one of its members.** A command resolves a path through `BundleIndex.getDeclarationsOfOwnPath` — main files plus declaring notes, the `ownPaths` of `getPaths` seen from the other side — and never through the member map, which is what the delete rule's "a file two bundles declare survives either" test reads: indexing a sidecar as a member of its own bundle would make it look like a shared dependent to that rule. Until that lookup existed every command answered `no bundle declared` for the sidecar, which for a binary-main bundle is the one file of the pair Obsidian can open at all.
...
- **The vault ROOT is a folder whose path prefix is EMPTY.** `dirname` answers `.` for a file at the top of the vault, so reading that as an ordinary folder path asks whether a member starts with `./` — false for every path a vault holds — and then slices those two characters off a path that never carried them. `bundle-operations.ts` asks `toFolderPathPrefix()` for both halves, so the prefix `isUnder` matches is the prefix `rebase` cuts and the two cannot drift; `bundle-declaration.ts` and `bundle-index.ts` name the same `VAULT_ROOT_FOLDER_PATH` for their own root checks. Until this was fixed a bundle declared by a root-level note propagated to NOTHING, and no test saw it because every suite sat the bundle in `Alpha/`.
- **Every integration suite waits from NODE, not just the capture ones.** The `## The screenshot capture suites` section explains why a long wait cannot live inside one `evalInObsidian` closure; the same applies to the behavioral `*.cross-platform.integration.test.ts` suites, and it is the Android leg that proves it. The lock, move and rename suites each declared modest in-eval ceilings that summed to well under the 30 s cap and still died as `EvalCapExceededError` on the emulator at 31–33 s, because the vault operations sharing that closure — the creations, the real rename, the plugin's transactional propagation — spend most of the budget themselves. Desktop passed throughout, so the lexical sum the lint rule checks is a floor and not a guarantee: what matters is that no single closure both *acts* and *waits*. The shape is a `pollInObsidian` whose `start` performs one operation and whose `poll` reads the vault and returns at once, with a Node-side `sleepInNode` for a settle and a generous Node-side `timeoutInMilliseconds` the cap does not bound.
- **A wait names an operation's LAST write, never its first observable effect.** Every bundle operation is several writes, and the one that finishes it is the declaration rewrite that points the result at its own files: `duplicateBundle` awaits every copy — the main, the declaring note, each relative member — and only then calls `rewriteBundleDeclaration`. So a wait on the copied file merely EXISTING accepts halfway through, in a window where the copy is carrying a verbatim copy of the ORIGINAL's declaration and therefore still names the original's main and members. Desktop hides the gap; on the emulator the read lands inside it, and the suite fails at a *different* assert on each run, which is the signature of a race rather than of a wrong expectation. Wait on the content the asserts read — `content.includes('diagram 1.png')`, not `!!getFileByPath(...)`.
...
- **No demonstration sidecar is EVER named after the file it declares.** Obsidian strips `.md` from every row it lists, so a `report.html.md` beside a `report.html` paints two rows labelled `report`, and nothing on screen says which is the main file and which the note declaring it. That is bad for a reader and worse for the claim, because this plugin's headline rule is that the `file-bundles` key marks a declaration and the file's NAME never does — a vault whose only sidecar is named after its main file reads as the naming convention `awesym/obsidian-sidecars` uses and this plugin's prior-art note distinguishes itself from. The demo vault and both capture suites all call theirs `Quarterly report.md`, and each says in prose or in a docblock why the name is free. The `Alpha/report.html.md` UNIT fixtures are the deliberate exception: they are the shape the plugin has to READ, and one of them asserts precisely that such a sidecar still works.

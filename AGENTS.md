...
- **The README has no screenshot block, and the community listing reads the README.** No `images/screenshots/` and no capture suites exist here yet; the README skeleton puts the gallery between the lead paragraph and `## Demo vault`. Wanted before the first release.

...
- `scripts/version.ts` deliberately carries **no** template-release guard. The sample-plugin-extended template ships a guard keyed to its own plugin id; retargeting that id while scaffolding re-arms it against the new plugin and makes every release throw. `alias-quick-switcher` and `advanced-markdown-export` both carry that defect today.

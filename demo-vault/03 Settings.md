...
- `excludedPathPatterns`
  - paths matching any of these are never treated as part of a bundle, whether they declare one or are declared by one. Matched against the whole vault-relative path, so a pattern can exclude a folder or a single file.
